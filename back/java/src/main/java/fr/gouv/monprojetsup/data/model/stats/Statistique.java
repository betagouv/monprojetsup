...
import static fr.gouv.monprojetsup.data.model.stats.PsupStatistiques.PRECISION_PERCENTILES;
...
public record Statistique(
    ...
    public static Statistique getStatAgregee(List<Statistique> toList) {
        if(toList.stream().anyMatch(e -> e.frequencesCumulees.length != PRECISION_PERCENTILES)) {
            throw new RuntimeException("Cannot merge stats of different sizes");
        }
        int[] frequencesCumulees = new int[PRECISION_PERCENTILES];
        for(int i = 0; i < PRECISION_PERCENTILES; i++) {
            int finalI = i;
    ...
    public int nb() {
        return frequencesCumulees == null ? 0 : frequencesCumulees[PRECISION_PERCENTILES - 1];
    }
